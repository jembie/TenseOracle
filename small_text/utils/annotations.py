...
def deprecated(
    func_or_class=None, deprecated_in=None, to_be_removed_in=None, replacement=None
):
    ...
    def _decorator(func_or_class):
        ...
        removal_version = (
            version.parse(to_be_removed_in) if to_be_removed_in is not None else None
        )
        current_version = get_version()

        if removal_version is not None and current_version >= removal_version:
            raise DeprecationError(
                f"The {subject} {func_or_class.__name__} should have been "
        ...
        def wrapper(*args, **kwargs):
            removed_in = (
                f" and will be removed in {str(removal_version)}"
            ...
            warnings.warn(
                f"The {subject} {func_or_class.__name__} has been deprecated in "
...
def experimental(func_or_class=None):
    ...
    def _decorator(func_or_class):
        ...
        def wrapper(*args, **kwargs):
            warnings.warn(
                f"The {subject} {func_or_class.__name__} is experimental "
...
def early_stopping_deprecation_warning(
