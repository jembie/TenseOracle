...
def check_window_based_predictions(predictions, last_predictions):
    if predictions is None or np.all(
        [predictions[i] is None for i in range(predictions.shape[0])]
    ):
        raise ValueError("Predictions must not be None!")
    elif (
        last_predictions is not None
        and predictions.shape[0] != last_predictions.shape[0]
    ):
        raise ValueError(
            f"Predictions must not differ in size: "
...
class DeltaFScore(StoppingCriterion):
    ...
    def __init__(self, num_classes, window_size=3, threshold=0.05):
        ...
        if num_classes != 2:
            raise ValueError(
                "DeltaFScore is only applicable for binary classifications "
    ...
    def stop(
        ...
        if self.last_predictions is None:
            ...
            disagreement_old_positive = (
                ((self.last_predictions == 1) & (predictions == 0)).astype(int).sum()
            )
            disagreement_new_positive = (
                ((self.last_predictions == 0) & (predictions == 1)).astype(int).sum()
            )

            denominator = (
                2 * agreement + disagreement_old_positive + disagreement_new_positive
            )
