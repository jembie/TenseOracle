...
OPTIONAL_DEPENDENCIES = dict({"hnswlib": "hnswlib", "setfit": "setfit"})
...
def check_optional_dependency(dependency_name):
    try:
        if dependency_name not in OPTIONAL_DEPENDENCIES.keys():
            raise ValueError(
                f"The given dependency '{dependency_name}' is not registered "
        ...
        exception_msg = (
            f"The optional dependency '{dependency_name}' is required "
