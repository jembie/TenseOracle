...
class PytorchModelSelectionMixin(object):
    ...
    def _perform_model_selection(self, optimizer, model_selection):
        ...
        if model_selection_result is not None:
            ...
            optimizer_path = model_selection_result.model_path.with_suffix(
                ".pt.optimizer"
            )
...
class PytorchClassifier(PytorchModelSelectionMixin, Classifier):
    ...
    def predict_proba(self, dataset, dropout_sampling=1):
        ...
        dataset_iter = dataloader(
            dataset.data, self.mini_batch_size, self._create_collate_fn(), train=False
        )
        ...
        with torch.no_grad():
            if dropout_sampling <= 1:
                ...
                return self._predict_proba_dropout_sampling(
                    dataset_iter, logits_transform, dropout_samples=dropout_sampling
                )
    ...
    def _predict_proba_dropout_sampling(
        ...
        raise NotImplementedError(
            "_predict_proba_dropout_sampling() needs to be implemented"
        )
    ...
    def _get_default_early_stopping(
        self,
        early_stopping,
        ...
        if early_stopping is None:
            ...
            if early_stopping_no_improvement != 5 or early_stopping_acc != -1:
                warnings.warn(
                    f"Both the fit() argument early_stopping and the __init__() "
