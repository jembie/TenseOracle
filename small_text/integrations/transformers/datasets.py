...
class TransformersDatasetView(PytorchDatasetView):
    def clone(self):
        if self.is_multi_label:
            data = [
                (
                    torch.clone(d[TransformersDataset.INDEX_TEXT]),
                    torch.clone(d[TransformersDataset.INDEX_MASK]),
                    d[TransformersDataset.INDEX_LABEL].copy(),
                )
                for d in self.data
            ]
        else:
            data = [
                (
                    torch.clone(d[TransformersDataset.INDEX_TEXT]),
                    torch.clone(d[TransformersDataset.INDEX_MASK]),
                    np.copy(d[TransformersDataset.INDEX_LABEL]),
                )
                for d in self.data
            ]

        target_labels = (
            None if self.dataset.track_target_labels else np.copy(self.target_labels)
        )
        return TransformersDataset(
            data, multi_label=self.is_multi_label, target_labels=target_labels
        )
...
class TransformersDataset(PytorchDataset):
    ...
    def clone(self):
        if self.is_multi_label:
            data = [
                (
                    torch.clone(d[self.INDEX_TEXT]),
                    torch.clone(d[self.INDEX_MASK]),
                    d[self.INDEX_LABEL].copy(),
                )
                for d in self._data
            ]
        else:
            data = [
                (
                    torch.clone(d[self.INDEX_TEXT]),
                    torch.clone(d[self.INDEX_MASK]),
                    np.copy(d[self.INDEX_LABEL]),
                )
                for d in self._data
            ]

        target_labels = (
            None if self.track_target_labels else np.copy(self._target_labels)
        )
        return TransformersDataset(
            data, multi_label=self.multi_label, target_labels=target_labels
        )
