...
def _get_arguments_for_from_pretrained_model(
    model_loading_strategy: ModelLoadingStrategy,
) -> PretrainedModelLoadingArguments:
...
def _initialize_transformer_components(
    transformer_model, num_classes: int, cache_dir: str
):
...
def _build_layer_specific_params(
    model: PreTrainedModel, base_lr: float, fine_tuning_arguments
):
    ...
    if fine_tuning_arguments.gradual_unfreezing >= len(layers):
        raise ValueError(
            "Invalid gradual unfreezing parameters: No trainable layers left."
        )

    params, start_layer = _adapt_layer_specific_params(
        layers, base_lr, fine_tuning_arguments
    )
...
def _adapt_layer_specific_params(
    layers: List[dict], base_lr: float, fine_tuning_arguments
):
    ...
    start_layer = (
        ...
        else max(0, total_layers - fine_tuning_arguments.gradual_unfreezing)
    )
...
def _check_for_completeness(model, layers, params, start_layer: int):
    ...
    if len(must_have_layer_ids - found_layer_ids) != 0:
        ...
        raise ValueError(
            f'Fine-tuning arguments are not supported for transformer model "{type(model).__name__}".'
        )
